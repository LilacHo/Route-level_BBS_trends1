...
data {
  ...
  array [ncounts] real<lower=0, upper=1> developed; // proportion developed cover
...
parameters {
  ...
  real gamma2; // effect of developed-cover proportion on log(lambda)
 ...
 //  noise = sdnoise*noise_raw;
  ...
  for(i in 1:ncounts){
    E[i] =  beta[route[i]] * (year[i]-fixedyear) + alpha[route[i]] + obs[observer[i]] + eta*firstyr[i]
            + gamma1*grassland[i] + gamma2*developed[i];
  }
  ...
  gamma2 ~ normal(0,1);// prior on developed-cover effect
...
// derived/reported quantities in the output summary
   ...
   gamma2_out = gamma2; // developed-cover effect on log(lambda)
