...
//
// Covariates are passed as a single array of K-vectors (one K-vector per
// observation, i.e. per route-year-count row) rather than as 16 separate
// arrays, so the model generalizes to any K without editing this file.
...
data {
  ...
  // cover types (range [0,1]); X[i] is the K-vector of covariate values for
  // observation i, aligned 1:1 with count/year/route/etc. above.
  array [ncounts] vector<lower=0, upper=1>[K] X;
