...
// with structures to support leave-future-out cross-validation of the
// next year's observations. Paired with models/slope_iCAR_route_ZINB_New.stan.
//
// Uses the same sum_to_zero_vector / target += ICAR reparameterization as
// slope_iCAR_route_NB_New.stan. Note that its plain-NB CV sibling,
// models/slope_iCAR_route_NB_cv.stan, still uses the older icar_normal_lpdf
// soft-constraint style (it predates the "New" reparameterization and was
// left as-is); since this is a new file we use the more efficient
// hard-constraint version throughout instead of matching that older style.
//
// The zero-inflation mixture is baked into the predictive log_lik (not just
// the training likelihood) so that cv_lppd is directly comparable between
// the NB and ZINB models in 3_compare_NB_ZINB.R. E_pred is left as the
// NB2 conditional mean (pre zero-inflation discount) to match the existing
// NB cv file's convention — a global theta rescales every prediction by the
// same constant (1-theta), so it does not change the rank order used for
// the Spearman correlation computed downstream.
//
...
// covariate effect on intercepts and slopes
 ...
 generated quantities {
  ...
  vector[ncounts_pred] E_pred;           // log_scale additive likelihood (NB2 conditional mean, pre zero-inflation)
...
// Predictions for nyears+1, with the zero-inflation mixture folded into the
// predictive log-likelihood (log_lik) so cv_lppd is comparable to the NB model.
  ...
  for(i in 1:ncounts_pred){
    ...
    if(count_pred[i] == 0){
      log_lik[i] = log_sum_exp(
        bernoulli_lpmf(1 | theta),
        bernoulli_lpmf(0 | theta) + neg_binomial_2_log_lpmf(0 | E_pred[i], phi)
      );
    } else {
      log_lik[i] = bernoulli_lpmf(0 | theta) + neg_binomial_2_log_lpmf(count_pred[i] | E_pred[i], phi);
    }
  }
