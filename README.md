...
4. For whichever of a small, named set of confirmed "stubborn" species/tags — Sharp-shinned Hawk, Hooded Merganser, Broad-winged Hawk, Belted Kingfisher (both `base` and `anthro` each), plus Western Meadowlark, Gray Catbird, Red Crossbill, and Wood Duck (`anthro` only each) — are still failing, gives them one more, more aggressive push (8000/8000, `adapt_delta = 0.95`) via `helper/stubborn_species_refit.R`'s `refit_stubborn_species()`, validated against all 11 species/tag combos in `tests/test_stubborn_species.R`.
